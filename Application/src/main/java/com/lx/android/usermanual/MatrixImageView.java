...
/**
 * Created by Micheal on 2016/5/19.
 */
...
public class MatrixImageView extends ImageView {
    ...
    /**  模板Matrix，用以初始化 */
    private Matrix mDefalutMatrix=new Matrix();
    ...
    MatrixTouchListener mListener=null;
        ...
        if(getWidth()==0){
          //  Log.d(TAG,"未测量");
            ...
            vto.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener()
            {
                ...
                {
                    initData();
                    mDefalutMatrix.set(getImageMatrix());
                    float[] values=new float[9];
                    mDefalutMatrix.getValues(values);
                    //图片宽度为屏幕宽度除缩放倍数
    ...
    private void initData() {
        //设置完图片后，获取该图片的坐标变换矩阵
      //  Log.d(TAG,"initData~~~");
        setImageMatrix(mDefalutMatrix);

        mMode = 0;
        mCurrentMatrix.reset();
        mCurrentMatrix.set(mDefalutMatrix);
...
//        private Matrix mCurrentMatrix = new Matrix();
        ...
        private boolean checkRest() {
            ...
            float scale=values[Matrix.MSCALE_X];
            //获取模板的X轴缩放级别，两者做比较
       //     Log.d(TAG,"checkRest: scale="+scale+" mScale="+mScale);
            return scale<mScale;
        }
        ...
        public void onDoubleClick(){
            ...
            mCurrentMatrix.set(mDefalutMatrix);//初始化Matrix
    ...
    private class  GestureListener extends SimpleOnGestureListener {
       ...
       //     Log.d(TAG,"onFling");
            ...
            boolean horizontalFlag=false;//是否水平滑动
            //获取水平移动距离
            if(beginX>endX) horizontalMove=beginX-endX;
            else horizontalMove=endX-beginX;
            //获取垂直移动距离
            if(beginY>endY) verticalMove=beginY-endY;
            else verticalMove=endY-beginY;
            //判断是水平还是垂直
            if(horizontalMove>verticalMove) horizontalFlag = true;
            ...
            }else if(endY-beginY>minMove&&Math.abs(velocityY)>minVelocity  && !horizontalFlag){
                //下滑
            }
